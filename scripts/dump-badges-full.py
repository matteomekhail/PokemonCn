#!/usr/bin/env python3
"""Dump each badge NCGR (62..77) at FULL 32×440 size with each palette to find
the right pairing and the location of the actual badge sprite within the strip."""
from pathlib import Path
import struct, zlib

REPO = Path(__file__).resolve().parents[1]
NARC = REPO / ".refs/pokeheartgold/files/a/0/4/9"
OUT = REPO / "scripts/_dump_badges_full"
...
def write_png(path, pixels, w, h, palette, scale=2):
...
# Render each badge NCGR with every of the first 16 palettes
for n in range(62, 78):
    w, h, px = ncgr_pixels(files[n])
    for p in range(16):
        pal = parse_nclr(files[p])
        if len(pal) < 16: pal = pal + [(0, 0, 0)] * (16 - len(pal))
        write_png(OUT / f"ncgr{n:02d}_pal{p:02d}.png", px, w, h, pal[:16], scale=2)
print(f"wrote {16 * 16} PNGs to {OUT}")
